...
export default function TeacherChatPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/chat/conversations").then(r => r.json()).then(d => {
      const convs: ConvData[] = d.conversations ?? []
      setConversations(convs)
      if (convs.length > 0) {
        setActiveContactId(convs[0].otherId)
  ...
  }, [contacts, conversations, search, filter])

  const activeContact = contacts.find(c => c.id === activeContactId)
    || (activeContactId ? { id: activeContactId, name: conversations.find(c => c.otherId === activeContactId)?.otherName ?? "", role: conversations.find(c => c.otherId === activeContactId)?.otherRole ?? "" } : null)
        ...
        <div className="w-72 flex-shrink-0 bg-white border-r border-[#E2E8F0] flex flex-col">
          <div className="p-4 space-y-3 border-b border-[#E2E8F0]">
            ...
            <div className="flex gap-1.5">
              {([["all", "Бүгд"], ["student", "Сурагч"], ["counselor", "Зөвлөх"]] as const).map(([key, label]) => (
                <button key={key} onClick={() => setFilter(key)}
                  className={cn("flex-1 py-1.5 rounded-lg text-xs font-medium transition-colors",
                    filter === key ? "bg-[#4A8C4D] text-white" : "bg-[#F8FAFC] text-[#64748B] hover:bg-[#E2E8F0]")}>
                  {label}
                </button>
              ))}
            </div>
              ...
              return (
                <button key={contact.id} onClick={() => handleSelectContact(contact)}
                  className={cn(
                    ...
                    isActive && "bg-[#EDFAEE] hover:bg-[#EDFAEE]"
                  )}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#64748B] truncate">{conv?.lastMessage || contact.grade || roleLabel[contact.role]}</p>
                  </div>
                  {(conv?.unread ?? 0) > 0 && (
                    <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#4A8C4D] text-white text-[10px] font-bold flex items-center justify-center">
                      {conv!.unread}
                    </span>
                  )}
                </button>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!activeContact ? (
            ...
            <>
                    ...
                    <div key={msg.id} className={cn("flex items-end gap-2", isMe ? "justify-end" : "justify-start")}>
                      ...
                      <div className={cn("max-w-sm flex flex-col gap-1", isMe ? "items-end" : "items-start")}>
                        <div className={cn(
                          ...
                          isMe ? "bg-[#4A8C4D] text-white rounded-br-sm" : "bg-white border border-[#E2E8F0] text-[#0F172A] rounded-bl-sm"
                        )}>{msg.content}</div>
              ...
              {/* Quick replies */}
              <div className="bg-white border-t border-[#E2E8F0] px-4 py-2 flex items-center gap-2 overflow-x-auto">
                <span className="text-xs text-[#64748B] flex-shrink-0">Хурдан хариу:</span>
                {["Ойлголоо", "Тийм, илгээнэ", "Баярлалаа", "Хожим ярья"].map(q => (
                  <button key={q} onClick={() => setInput(q)}
                    className="text-xs bg-[#F8FAFC] border border-[#E2E8F0] hover:border-[#4A8C4D] text-[#64748B] hover:text-[#4A8C4D] px-3 py-1.5 rounded-full transition-colors flex-shrink-0">
                    {q}
                  </button>
                ))}
              </div>
                ...
                <div className="flex items-center gap-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl px-4 py-3">
                  ...
                  <button onClick={handleSend}
                    className={cn("flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all",
                      input.trim() ? "bg-[#4A8C4D] text-white hover:bg-[#3d7340]" : "bg-[#E2E8F0] text-[#64748B] cursor-not-allowed")}>
