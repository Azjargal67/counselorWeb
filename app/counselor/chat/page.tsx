...
import {
  Search, Send, CheckCheck, MoreHorizontal,
...
export default function CounselorChatPage() {
  ...
  const activeContact = contacts.find(c => c.id === activeContactId)
    || (activeContactId ? conversations.find(c => c.otherId === activeContactId) && { id: activeContactId, name: conversations.find(c => c.otherId === activeContactId)?.otherName ?? "", role: conversations.find(c => c.otherId === activeContactId)?.otherRole ?? "" } : null)
        ...
        <div className="w-72 flex-shrink-0 bg-white border-r border-[#E2E8F0] flex flex-col">
          <div className="p-4 space-y-3 border-b border-[#E2E8F0]">
            ...
            <div className="flex gap-1.5">
              {([["all", "Бүгд"], ["student", "Сурагч"], ["teacher", "Багш"]] as const).map(([key, label]) => (
                <button key={key} onClick={() => setFilter(key)}
                  className={cn("flex-1 py-1.5 rounded-lg text-xs font-medium transition-colors",
                    filter === key ? "bg-[#4361EE] text-white" : "bg-[#F8FAFC] text-[#64748B] hover:bg-[#E2E8F0]")}>
                  {label}
                </button>
              ))}
            </div>
          ...
          {!activeContact ? (
            ...
            <>
                  ...
                  {activeContact.role === "student" && (
                    <>
                      <button className="flex items-center gap-1.5 text-xs font-medium text-[#3451D1] bg-[#EEF1FD] hover:bg-[#D4DCEF] px-3 py-2 rounded-lg transition-colors">
                        <ClipboardList className="w-3.5 h-3.5" /> Даалгавар
                      </button>
                      <button className="flex items-center gap-1.5 text-xs font-medium text-[#4A8C4D] bg-[#DFEDE0] hover:bg-[#C8DFD0] px-3 py-2 rounded-lg transition-colors">
                        <FileText className="w-3.5 h-3.5" /> Материал
                      </button>
                    </>
                  )}
              ...
              <div className="bg-white border-t border-[#E2E8F0] px-4 py-2 flex items-center gap-2 overflow-x-auto">
                ...
                {["Ойлголоо", "Deadline-ийг тэмдэглэлээ", "Баярлалаа"].map(q => (
                  <button key={q} onClick={() => setInput(q)}
                    className="text-xs bg-[#F8FAFC] border border-[#E2E8F0] hover:border-[#4361EE] text-[#64748B] hover:text-[#3451D1] px-3 py-1.5 rounded-full transition-colors flex-shrink-0">
                    {q}
                  </button>
                ))}
              </div>
